"""
FastAPI wrapper — runs the expense agent locally and accepts Pub/Sub push messages.
...
HOW TO RUN LOCALLY:
  ...
  4. Test:    curl -X POST http://localhost:8080/apps/expense_agent/trigger/pubsub \
                -H "Content-Type: application/json" \
                -d '{"message": {"data": ""}, "description": "Team lunch $45"}'
"""
...
async def pubsub_trigger(request: Request):
    """Receive a Pub/Sub push message and run it through the expense agent."""
    try:
    ...
    try:
        ...
        if "RESOURCE_EXHAUSTED" in err or "429" in err:
            # Gemini quota hit — route to human review without AI analysis
            result_text = (
                "HUMAN REVIEW REQUIRED: AI analysis unavailable (Gemini quota exceeded). "
                "Please review this expense manually."
            )
        else:
            raise
